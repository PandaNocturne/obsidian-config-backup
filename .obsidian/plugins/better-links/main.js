...
function findLinkAtOffset(lineText, offset2, settings) {
  var _a;
  const matches = collectMatches(lineText, settings);
  return (_a = matches.find((match) => offset2 >= match.start && offset2 < match.end)) != null ? _a : null;
}
...
var LinkDestinationSuggest = class extends import_obsidian3.AbstractInputSuggest {
  ...
  renderFileSuggestion(item, el) {
    ...
    const displayName = isMarkdown ? item.file.basename : item.file.name;
    ...
    if (item.match && item.match.matches.length > 0) {
      ...
      if (basenameMatch) {
        (0, import_obsidian3.renderResults)(titleEl, displayName, basenameMatch);
      } else {
