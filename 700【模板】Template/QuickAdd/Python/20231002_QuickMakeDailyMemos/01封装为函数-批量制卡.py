...
def extraction_text_and_file(file_path_content,output_ex_jsons, filenames,output_dir,file_path_toc,file_toc_yaml):
    
    i = 0
    with open(file_path_toc, 'w', encoding='utf-8') as file:
        file.write(file_toc_yaml)
    ...
    for output_ex_json in output_ex_jsons:
        ...
        # 将提取的元素保存到新的md文件中
        output_json_path = f"{output_dir}\{filename}.md"
        with open(output_json_path, 'w', encoding='utf-8') as file:
            file.write('\n'.join(save_data))
        with open(file_path_toc, 'a+', encoding='utf-8') as file:
            file.write(f"- [[{filename}]]\n")



def all_extraction_frame(file_path,file_dir,file_path_toc,file_toc_yaml):
    # 读取.md文件
    ...
    # 创建输出目录
    extraction_text_and_file(file_path_content,output_ex_jsons,filenames,file_dir,file_path_toc,file_toc_yaml)

# %%%
file_path = r"D:\PandaNotes\🗃卡片笔记盒🗃\每日话题记录\📅20230928 1.md"
file_basename = os.path.basename(file_path).replace(".md", "")
file_dir = os.path.dirname(file_path)
file_path_toc = f"{file_dir}\{file_basename}-TOC.md"

file_toc_yaml = "---\ncssclasses:\n  - list-cards\n  - cards-cols-3\n---\n"

all_extraction_frame(file_path,file_dir,file_path_toc,file_toc_yaml)