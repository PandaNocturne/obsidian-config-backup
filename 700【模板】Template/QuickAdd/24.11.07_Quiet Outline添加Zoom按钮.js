...
function addButtonToTreeItems() {
  ...
  const treeItems = document.querySelectorAll('.quiet-outline .n-config-provider .n-tree-node-wrapper');
...
function executeCommand(treeItem) {
  ...
  const treeItemSelf = treeItem.querySelector('.n-tree-node-content');
  if (treeItemSelf) {
    treeItemSelf.click();
  }
...
document.querySelectorAll('.quiet-outline .n-config-provider').forEach(container => {
  observer.observe(container, { childList: true, subtree: true });
});

// 初始调用
app.commands.executeCommandById("obsidian-quiet-outline:quiet-outline");
